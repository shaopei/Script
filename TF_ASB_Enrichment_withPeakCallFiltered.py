...
####
data = np.loadtxt('d.SymCals.all_withTFPeakCall.txt', dtype=str ,delimiter='\t', usecols=xrange(224), skiprows=0)
YY1_ENCFF002CIC = get_sno_in_region_vector('ENCFF002CIC')
YY1_ENCFF002CIC_array = np.concatenate((np.array('ENCFF002CIC').reshape(1,1),YY1_ENCFF002CIC[1].reshape(2024311,1)))
combined_matrix =np.concatenate((data,YY1_ENCFF002CIC_array),axis=1)
np.savetxt('d.SymCals.all_withTFPeakCall.txt', combined_matrix, fmt='%s', delimiter='\t')
...
snp_f='d.SymCals.all.YY1_all_W.txt'
...
#peak_calls = b[1:,2:]
peak_calls = np.loadtxt('d.SymCals.all_withTFPeakCall.txt', dtype=str ,delimiter='\t', usecols=xrange(120,225), skiprows=1)
peak_calls_file_names = open('d.SymCals.all_withTFPeakCall.txt','U').readline().strip().split('\t')[120:225]
