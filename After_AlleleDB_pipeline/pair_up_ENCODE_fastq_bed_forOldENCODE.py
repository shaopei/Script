...
fastq_metadata_fp = '/workdir/sc2457/ENCODE/reDoTF/metadata.tsv' # for downloaded fastq
...
CNVFiltered_counts_fp = '/workdir/sc2457/ENCODE/reDoTF/reDoTF_counts_file/1_CNVFiltered_counts_bed/'
...
import glob
file_names = glob.glob('*CNVFiltered_counts.bed')
Exp_Acc_processded=set()
with open('make_2_CNV.PeakFiltered_counts.bed.sh','w') as out:
    out.write('mkdir 2_CNV.PeakFiltered_counts_bed\n')
    for f in file_names:
        Exp_acc = f.split('.')[1]
        Exp_Acc_processded.add(Exp_acc)
        try:
            out.write('intersectBed -wa -a '+CNVFiltered_counts_fp+ExperimentAcc_Target_dic[Exp_acc]+'.'+Exp_acc+'.CNVFiltered_counts.bed -b '
                      +' '.join(bed_files_fp+b for b in ExperimentAcc_BedFileAcc_dic[Exp_acc])+' > 2_CNV.PeakFiltered_counts_bed/'+Exp_acc+'.CNV.PeakFiltered_counts.bed\n')
        except KeyError:
            try:
                out.write('intersectBed -wa -a '+CNVFiltered_counts_fp+FastqFileAcc_Target_dic[Exp_acc]+'.'+Exp_acc+'.CNVFiltered_counts.bed -b '
                      +' '.join(bed_files_fp+b for b in ExperimentAcc_BedFileAcc_dic[FastqFileAcc_ExperimentAcc_dic[Exp_acc]])+' > 2_CNV.PeakFiltered_counts_bed/'+Exp_acc+'.CNV.PeakFiltered_counts.bed\n')
            except KeyError:
                out.write('cp '+CNVFiltered_counts_fp+f+' 2_CNV.PeakFiltered_counts_bed/'+Exp_acc+'.CNVFiltered_counts.bed\n' )
