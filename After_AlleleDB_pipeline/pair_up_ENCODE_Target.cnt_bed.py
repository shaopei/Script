...
CNVFiltered_counts_fp = '1_CNVFiltered_counts_bed/'
...
#import os
#os.chdir(CNVFiltered_counts_fp)
#import glob
#file_names = glob.glob('*CNVFiltered_counts.bed')
#os.chdir('../')
#Exp_Acc_processded=set()
#with open('make_2_CNV.PeakFiltered_counts.bed_Target.sh','w') as out:
#    out.write('mkdir 2_CNV.PeakFiltered_counts_bed\n')
#    for f in file_names:
#        target = f.split('.')[0]
#        try:
#            out.write('intersectBed -wa -a '+CNVFiltered_counts_fp+f+' -b '
#                      +' '.join(bed_files_fp+b for b in Target_BedFileAcc_dic[target])+' > 2_CNV.PeakFiltered_counts_bed/'+Exp_acc+'.CNV.PeakFiltered_counts.bed\n')
#        except KeyError:
#            out.write('cp '+CNVFiltered_counts_fp+f+' 2_CNV.PeakFiltered_counts_bed/'+target+'.CNVFiltered_counts.bed\n' )

with open('make_2_CNV.PeakFiltered_counts.bed_Target.sh','w') as out:
    for target in Target_FastqFileAcc_dic:
        try:
            out.write('intersectBed -wa -a '+CNVFiltered_counts_fp+target+'.CNVFiltered_counts.bed -b '
                      +' '.join(bed_files_fp+b for b in Target_BedFileAcc_dic[target])+' > 2_CNV.PeakFiltered_counts_bed/'+target+'.CNV.PeakFiltered_counts.bed\n')
        except KeyError:
            out.write('cp '+CNVFiltered_counts_fp+target+'.CNVFiltered_counts.bed 2_CNV.PeakFiltered_counts_bed/'+target+'.CNVFiltered_counts.bed\n' )
