...
def read_file_to_nparray (snp_f):
    ...
    winning_parent = np.loadtxt(snp_f, dtype=str ,delimiter='\t', usecols=[13], skiprows=1)
    return temp[:,0], temp[:,1], winning_parent #chr, snppos, winning_parent
...
plus_chrom, plus_snppos, plus_winning_parent = read_file_to_nparray(plus_groseq_fp)
minus_chrom, minus_snppos, minus_winning_parent = read_file_to_nparray(minus_groseq_fp)
...
def if_snp_in_a_region(rChrom, rStart, rEnd, strand):
    ...
    if strand == 'plus':
        sChrom,snppos, winning_parent = plus_chrom, plus_snppos, plus_winning_parent
    else:
        sChrom,snppos, winning_parent = minus_chrom, minus_snppos, minus_winning_parent
...
def plus_Tss_three_stage_check(pChrom, pStart, pEnd, d_s0, d_s1, d_l, d_after_rStart=20, s0_count=0, s1_count=0, l_count=0):
    pw_use = None
    pt, ps, pw = if_snp_in_a_region(pChrom, pStart+d_after_rStart , pEnd + d_s0, 'plus')
    if pt.sum() > 0:
...
def minus_Tss_three_stage_check(mChrom, mStart, mEnd, d_s0, d_s1, d_l, d_after_rStart=20, s0_count=0, s1_count=0, l_count=0):
    ...
    if mt.sum() > 0:
        ...
        mt, ms, mw = if_snp_in_a_region(mChrom, mStart - d_s1 , mStart - d_s0, 'minus')
        if mt.sum() > 0:
            mw_use = mw[-1]  #use last one
            s1_count += 1
        #    print 'strict1',s1_count , mw,'mw_use:', mw_use, mChrom, mStart, mEnd
        else:
            mt, ms, mw = if_snp_in_a_region(mChrom, mStart - d_l , mStart - d_s1, 'minus')
            if mt.sum() > 0:
                mw_use = mw[-1]  #use last one
                l_count += 1
        #        print 'loose',l_count , mw,'mw_use:', mw_use, mChrom, mStart, mEnd
    return mt, ms, mw, mw_use
    ...
    with open(out_fp, 'w') as out:
        ...
        for i in xrange(len(tss_plus)):
            ...
            try:
                ...
                pt, ps, pw, pw_use = plus_Tss_three_stage_check(pChrom, pStart, pEnd, d_s0, d_s1, d_l,d_after_rStart)
                
                mChrom, mStart, mEnd = tss_minus[i][0:3]
                mChrom = int(mChrom.split('chr')[-1])
                assert mChrom == pChrom
                mStart, mEnd = int(mStart) , int(mEnd)
                mt, ms, mw, mw_use = minus_Tss_three_stage_check(mChrom, mStart, mEnd, d_s0, d_s1, d_l,d_after_rStart)
                
                if min(pt.sum(),mt.sum()) >= 1:
                    count += 1
                    if pw_use == mw_use:
                        Con_count +=1
                        out.write('\t'.join(['chr'+str(pChrom), str(min(pStart,mStart) - d_region), str(max(pEnd, mEnd) + d_region),'C', pw_use, mw_use])
                                  +'\n')
                    elif pw_use != mw_use:
                        Dis_count +=1
                        out.write('\t'.join(['chr'+str(pChrom), str(min(pStart,mStart) - d_region), str(max(pEnd, mEnd) + d_region),'D', pw_use, mw_use])
                                  +'\n')
